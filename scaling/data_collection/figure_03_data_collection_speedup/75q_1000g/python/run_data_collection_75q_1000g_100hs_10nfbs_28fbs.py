...
"""
Data collection for 75-qubit / 1000-gate circuits (100 hyper-samples, 10nfbs_28fbs).
...
# qubits_per_batch must sum to nqubits (75): 6 batches → 4×10, 7, 28
# shots_per_batch length must equal num_batches (6): 5×1, 100
RUNS = [
    {'nqubits': 75, 'ngates': 1000, 'num_hyper_samples': 100,
     'shots_per_batch': '1,1,1,1,1,100', 'qubits_per_batch': '10,10,10,10,7,28'},
]
...
def main():
    parser = argparse.ArgumentParser(
        description='Run 75q/1000g data collection over pre-generated circuits')
    default_circuit_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'circuits', '75q_1000g')
    parser.add_argument('--circuit-dir', type=str, default=default_circuit_dir,
                        help='Directory with pre-generated circuits (from generate_circuits.py). '
                             'Default: data_collection/circuits/75q_1000g')
