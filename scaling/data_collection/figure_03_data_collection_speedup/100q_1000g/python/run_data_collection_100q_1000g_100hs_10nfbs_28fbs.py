...
"""
Data collection for 100-qubit / 1000-gate circuits (100 hyper-samples, 10nfbs_28fbs).
...
# qubits_per_batch must sum to nqubits (100): 9 batches → 7×10, 2, 28
# shots_per_batch length must equal num_batches (9): 8×1, 100
RUNS = [
    {'nqubits': 100, 'ngates': 1000, 'num_hyper_samples': 100,
     'shots_per_batch': '1,1,1,1,1,1,1,1,100', 'qubits_per_batch': '10,10,10,10,10,10,10,2,28'},
]
...
def main():
    parser = argparse.ArgumentParser(
        description='Run 100q/1000g data collection over pre-generated circuits')
    default_circuit_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'circuits', '100q_1000g')
    parser.add_argument('--circuit-dir', type=str, default=default_circuit_dir,
                        help='Directory with pre-generated circuits (from generate_circuits.py). '
                             'Default: data_collection/circuits/100q_1000g')
